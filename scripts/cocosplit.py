...
def main(args):
    ...
    with open(args.annotations, 'rt', encoding='UTF-8') as annotations:
        ...
        if args.multi_class:
            ...
            save_coco(args.train, licenses, filter_images(images, X_train.reshape(-1)), X_train.reshape(-1).tolist(), categories)
            save_coco(args.test, licenses,  filter_images(images, X_test.reshape(-1)), X_test.reshape(-1).tolist(), categories)
            ...
            save_coco(args.train, licenses, X_train, anns_train, categories)
            save_coco(args.test, licenses, X_test, anns_test, categories)
